...
interface PackageBrowserProps {
  ...
  currentPackageIds?: { SALE?: string; RENT?: string };
  /** @deprecated use currentPackageIds */
...
export default function PackageBrowser({
  ...
  const setCategory = (next: PackageCategory) => {
  ...
  return (
    <div className="space-y-4">
      <div className="flex gap-2">
        {(["SALE", "RENT"] as PackageCategory[]).map((cat) => (
          <Button
            ...
            className={category === cat ? "bg-green-600 hover:bg-green-700 text-white" : ""}
            onClick={() => setCategory(cat)}
          >
            {cat === "SALE" ? "Sale (Buy listings)" : "Rent (Long Rent)"}
          </Button>
        ))}
      </div>
      ...
      <p className="text-sm text-gray-500">
        {category === "SALE"
          ? "Sale packages let you publish Buy listings."
          : "Rent packages let you publish Long Rent listings. Short stay does not need a package."}
      </p>

      {filtered.length === 0 ? (
        <p className="text-gray-400 py-8 text-center">
          No {category === "SALE" ? "Sale" : "Rent"} packages available at the moment.
        </p>
      ) : (
