...
import {
  ...
  MAX_BED_OPTIONS,
  ADDED_OPTIONS,
  DEFAULT_BUY_SEARCH_FILTERS,
} from "@/lib/buySearch";

interface BuyResultsFilterBarProps {
  filters: BuySearchFilters;
  onChange: (next: BuySearchFilters) => void;
  onSearch: (next?: BuySearchFilters) => void;
}
...
export default function BuyResultsFilterBar({
  filters,
  onChange,
  onSearch,
}: BuyResultsFilterBarProps) {
  const [showModal, setShowModal] = useState(false);

  const patchAndSearch = (patch: Partial<BuySearchFilters>) => {
        ...
        <div className="flex items-center justify-between w-full min-w-0 overflow-x-auto no-scrollbar gap-2 lg:gap-4">
          ...
          <div className="flex items-center gap-3 shrink-0">
            <FilterLabel
              ...
              options={PRICE_OPTIONS.filter((o) => o.value)}
            ...
            <FilterLabel
              ...
              options={MAX_PRICE_OPTIONS.filter((o) => o.value)}
