...
interface BuySearchFiltersFormProps {
  initial: BuySearchFilters;
}
...
export default function BuySearchFiltersForm({
  initial,
}: BuySearchFiltersFormProps) {
  const router = useRouter();
  const [filters, setFilters] = useState<BuySearchFilters>(initial);

  const update = (patch: Partial<BuySearchFilters>) =>
  ...
  return (
    ...
    >
      <h1 className="text-2xl sm:text-3xl font-bold text-slate-900 mb-8">
        Find property for sale{filters.location ? ` in ${filters.location}` : ""}
      </h1>
