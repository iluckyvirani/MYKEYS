...
function userHasAgentRole(roles: string[] | undefined) {
  return Array.isArray(roles) && roles.some((role) => role === "AGENT" || role === "ADMIN");
}
...
export default function OwnerPackagesPage() {
  ...
  useEffect(() => {
    ...
    if (!token) {
      router.replace("/login?redirect=/agent/packages");
      setAuthState("guest");
      return;
    }

    const user = getStoredUserFromLocalStorage();
    if (userHasAgentRole(user?.roles)) {
        ...
        <>
          ...
          <div className="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-white rounded-lg shadow-xl p-6 z-50 max-w-sm w-full mx-4">
            ...
            <p className="text-gray-600 mb-6">
              You are signed in as a guest user. Become an Agent to view and subscribe to listing
              packages.
            </p>
            <div className="flex gap-3">
              ...
              >
                {becomingOwner ? "Processing..." : "Become Agent"}
              </Button>
            </div>
            {becomeOwnerError && (
