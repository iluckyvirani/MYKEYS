...
export default function UserServicesPage() {
  ...
  const handleSubmitBooking = async () => {
    ...
    if (!addrHouseNo.trim() || !addrStreet.trim() || !addrCity.trim() || !addrPostcode.trim()) {
      toast({ title: "Address incomplete", description: "Please fill in house number, street, city and postcode.", variant: "destructive" });
      return;
    }
              ...
              <div className="flex items-start gap-4">
                ...
                <div className="flex-1 min-w-0">
                    ...
                    <div>
                      ...
                      <div className="flex flex-wrap items-center gap-x-3 gap-y-1 mt-1 text-sm text-gray-600">
                        <span className="flex items-center gap-1">
                          ...
                          {selectedProviderView.rating.toFixed(1)}
                          <span className="text-gray-400">({selectedProviderView.reviews} reviews)</span>
                        </span>
                        <span className="flex items-center gap-1">
                          <MapPin className="w-3.5 h-3.5 text-gray-400" />
                          {selectedProviderView.location || "Location not specified"}
                        </span>
                        <span className="text-gray-500">{selectedProviderView.completedBookings} jobs done</span>
                      </div>
                    </div>
                    {selectedProviderView.location && (
                      <a
                        href={`https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(selectedProviderView.location)}`}
                        target="_blank"
                        rel="noopener noreferrer"
                        className="flex items-center gap-1.5 text-xs font-medium text-blue-600 bg-blue-50 border border-blue-200 px-3 py-1.5 rounded-full hover:bg-blue-100 transition-colors shrink-0"
                      >
                  ...
                  {selectedProviderView.bio && (
                    <p className="text-sm text-gray-500 mt-2">{selectedProviderView.bio}</p>
                  )}
                  {selectedProviderView.serviceAreas.length > 0 && (
                    <div className="flex flex-wrap items-center gap-1.5 mt-3">
                      <span className="text-xs text-gray-500 font-medium">Serves:</span>
                      {selectedProviderView.serviceAreas.map((area) => (
                        <span key={area} className="px-2 py-0.5 bg-green-50 text-green-700 border border-green-100 text-xs rounded-full">{area}</span>
                      ))}
                    </div>
                  )}
                  {selectedProviderView.instantBooking && (
                    <div className="mt-2">
          ...
          <div className="bg-white rounded-[5px] max-w-2xl w-full p-6 max-h-[90vh] overflow-y-auto">
            ...
            <div className="mb-5">
              ...
              <input
                ...
                placeholder="Nearby landmark (optional — e.g. near Costa Coffee)"
                className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-green-500 mb-3"
              />
