    ...
    <DashboardLayout defaultRole="user">
      <div className="mb-6 flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-500 text-sm mt-1">Your conversations with property owners</p>
        </div>
        <Link href="/dashboard/inquiries/labels" className="text-sm text-green-600 hover:underline flex items-center gap-1">
      ...
      <div className="flex gap-2 mb-4 flex-wrap">
        <div className="relative flex-1 min-w-[200px]">
          ...
          <Input placeholder="Search by property or message..." className="pl-10" value={search} onChange={(e) => setSearch(e.target.value)} />
        </div>

        {/* Label filter */}
        <div className="relative">
          <button
            onClick={() => { setShowLabelDropdown(v => !v); setShowSortDropdown(false); }}
            className="flex items-center gap-2 px-3 py-2 border rounded text-sm text-gray-600 bg-white hover:bg-gray-50"
          >
          ...
          {showLabelDropdown && (
            <div className="absolute right-0 mt-1 w-52 bg-white border rounded-[5px] shadow-lg z-20 py-1">
              ...
              {LABELS.map(l => (
                <button key={l} onClick={() => { setLabelFilter(l); setShowLabelDropdown(false); }} className={`w-full text-left px-3 py-2 text-sm hover:bg-gray-50 ${labelFilter === l ? "font-semibold" : ""}`}>{l}</button>
              ))}
            </div>
          )}
        </div>
        ...
        <div className="relative">
          <button
            onClick={() => { setShowSortDropdown(v => !v); setShowLabelDropdown(false); }}
            className="flex items-center gap-2 px-3 py-2 border rounded text-sm text-gray-600 bg-white hover:bg-gray-50"
          >
          ...
          {showSortDropdown && (
            <div className="absolute right-0 mt-1 w-44 bg-white border rounded-[5px] shadow-lg z-20 py-1">
              {(Object.keys(SORT_LABELS) as SortKey[]).map(s => (
                <button key={s} onClick={() => { setSort(s); setShowSortDropdown(false); }} className={`w-full text-left px-3 py-2 text-sm hover:bg-gray-50 ${sort === s ? "font-semibold" : ""}`}>{SORT_LABELS[s]}</button>
              ))}
            </div>
          )}
        </div>
